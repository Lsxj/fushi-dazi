import { CATEGORIES, getPrimaryMembers } from '../../data/categories'
import { calcAgeMonths, BabyProfile } from '../../utils/planner'
...
function recommendMealsForAge(months: number): number {
...
function getMealBasis(months: number): string {
  if (months < 6) return '4-6月,以奶为主试少量'
  if (months < 7) return '6-7月,1-2顿'
  if (months < 10) return '7-9月,2顿'
  return '10月+,3顿'
}
...
Page({
  ...
  buildGroups() {
    const groups: CatGroup[] = CATEGORIES
      .filter(cat => !cat.noAllergyTracking)
      .map(cat => {
        // 排敏档案只列 primary：同源变体（鸡肝/排骨…）跟随主食材，勾主食材即代表整组通过
        const items: IngItem[] = getPrimaryMembers(cat.id).map(name => ({ name, checked: false }))
        return {
          id: cat.id,
          name: cat.name,
  ...
  onBirthday(e: any) {
    ...
    const months = calcAgeMonths(value)
  ...
  recalcGroups(groups: CatGroup[]) {
    let totalChecked = 0
    const openedSet = new Set<string>()
    const next = groups.map(g => {
      const checkedCount = g.items.filter(i => i.checked).length
      if (checkedCount > 0) openedSet.add(g.id)
