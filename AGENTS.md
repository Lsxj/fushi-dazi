...
- Put shared product rules in `utils/`. Put MCP orchestration in `mcp-server/src/domain/`. Keep MCP registration and Zod contracts in `mcp-server/src/tools/`.
...
The command covers mini-program regression tests, MCP TypeScript, Vitest coverage thresholds, MCP smoke tests, and the end-to-end agent flow.
