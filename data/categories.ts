...
export interface Category {
  ...
  /**
   * 全量食材名。同时是「食材 → 品类」映射源（getCategoryByFood）、
   * 库存/购物 picker 的候选池、recipes 食材名的合法性依据。
   * 删成员会让该食材退化成"未知食材"（analyzeSuspects 判高度可疑 + planner 取不到品类），
   * 因此想让某食材在排敏 UI 里隐身，用 variants 而不是从这里删。
   */
  members: string[]
  /**
   * 同源部位/变体：variant 食材名 → 主食材名。
   * 与主食材同物种、共享过敏原，不构成独立排敏项。
   * 排敏语义（welcome 勾选 / profile 品类展开 / planner 推荐）只看 primary；
   * 库存、购物、食谱仍可自由使用 variant。
   */
  variants?: Record<string, string>
  mainCategory: 'staple' | 'protein' | 'veg' | 'fruit' | 'oil' | 'condiment' | 'product' | 'preprocessed'
  noAllergyTracking?: boolean
  /** 常见食物过敏原分组；用于优先推荐和更明确的风险提示，不表示一定会过敏。 */
  commonAllergen?: boolean
}

export const SHOP_AREA_LABEL: Record<string, string> = {
  staple: '主食/谷物区',
  protein: '肉禽鱼蛋区',
  veg: '蔬果区',
  fruit: '蔬果区',
...
export function getShopAreaForFood(foodName: string): string {
  const cat = getCategoryByFood(foodName)
  if (!cat) return '其他'
  return SHOP_AREA_LABEL[cat.mainCategory] || '其他'
}

export const CATEGORIES: Category[] = [
  // 分类仅用于档案导航和推荐；每一种新食物仍需单独确认，不能用“同类已通过”替代。
  // ===== 谷薯类 =====
...
export function getCategoryByFood(foodName: string): Category | undefined {
  for (const c of CATEGORIES) {
    if (c.members.includes(foodName)) return c
  }
  const customs = getCustomFoods()
  const cf = customs.find(f => f.name === foodName)
  if (cf) return CATEGORIES.find(c => c.id === cf.categoryId)
  return undefined
}

export function getAllMembers(categoryId: string): string[] {
  const cat = CATEGORIES.find(c => c.id === categoryId)
  const builtin = cat?.members || []
  const customs = getCustomFoods().filter(f => f.categoryId === categoryId).map(f => f.name)
  return [...builtin, ...customs]
}

/** 该食材是否为同源变体（与主食材共享过敏原，不独立排敏） */
export function isVariantFood(foodName: string): boolean {
  return CATEGORIES.some(c => !!c.variants && foodName in c.variants)
}

/** 变体食材的主食材名；非变体返回 undefined */
export function getParentFood(foodName: string): string | undefined {
  for (const c of CATEGORIES) {
...
/** 主食材下挂的所有同源变体 */
export function getVariantsOf(foodName: string): string[] {
  for (const c of CATEGORIES) {
...
/**
 * 品类内的排敏候选（primary only，剔除同源变体）。
 * 所有「排敏」语义的 UI / 推荐逻辑都应该用这个，而不是 members。
 */
export function getPrimaryMembers(categoryId: string): string[] {
  const cat = CATEGORIES.find(c => c.id === categoryId)
  if (!cat) return []
  const customs = getCustomFoods().filter(f => f.categoryId === categoryId).map(f => f.name)
