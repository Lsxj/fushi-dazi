...
exports.getAllIngredientNames = getAllIngredientNames;
...
function getIngredient(name) {
    const builtin = exports.INGREDIENTS.find(i => i.name === name);
    if (builtin)
        return builtin;
    const custom = (0, customFoods_1.getCustomFoods)().find(f => f.name === name);
    if (custom) {
        return {
            name: custom.name,
...
function getAllIngredientNames() {
    const builtin = exports.INGREDIENTS.map(i => i.name);
    const customs = (0, customFoods_1.getCustomFoods)().map(f => f.name);
