...
exports.getStorageGuide = getStorageGuide;
exports.getDefaultLocationFor = getDefaultLocationFor;
...
const LOC_LABEL = { frozen: '冷冻', refrigerated: '冷藏', room: '室温' };
function getStorageGuide(foodName) {
    const ing = (0, ingredients_1.getIngredient)(foodName);
    const cat = (0, categories_1.getCategoryByFood)(foodName);
    const catGuide = cat ? CATEGORY_GUIDE[cat.id] : null;
    if (ing) {
        const days = [];
        if (ing.shelfLifeDays.frozen)
            days.push(`冷冻 ${ing.shelfLifeDays.frozen}天`);
        if (ing.shelfLifeDays.refrigerated)
            days.push(`冷藏 ${ing.shelfLifeDays.refrigerated}天`);
        if (ing.shelfLifeDays.room)
            days.push(`室温 ${ing.shelfLifeDays.room}天`);
        return {
...
function getDefaultLocationFor(foodName) {
    const ing = (0, ingredients_1.getIngredient)(foodName);
    if (ing)
        return ing.defaultStorage;
    const cat = (0, categories_1.getCategoryByFood)(foodName);
    if (!cat)
        return 'refrigerated';
    ...
    if (!guide)
        return 'refrigerated';
    if (guide.location.includes('冷冻'))
        return 'frozen';
    if (guide.location.includes('室温'))
        return 'room';
    return 'refrigerated';
}
