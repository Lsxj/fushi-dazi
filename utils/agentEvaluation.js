...
exports.evaluateAgenticWorkflow = evaluateAgenticWorkflow;
const safety_1 = require("./safety");
const agentRouting_1 = require("./agentRouting");
const baseProfile = {
    ageMonths: 10,
...
function evaluateAgenticWorkflow(router = agentRouting_1.routeAgentRequest, evaluatedAt = new Date()) {
    const cases = evaluationCases.map((definition) => {
        ...
        const groundingProxyPassed = actualTool !== null &&
            evidenceByTool[actualTool] === definition.evidenceSource;
        let actualSafety = null;
        if (definition.expectedSafety !== null &&
            route?.name === 'check_food_safety') {
            const foods = Array.isArray(route.input.foods)
                ? route.input.foods.filter((food) => typeof food === 'string')
                : [];
            if (foods.length > 0) {
                actualSafety = (0, safety_1.checkFoodsSafety)(foods, definition.profile ?? baseProfile).safe
                    ? 'allow'
                    : 'block';
            }
        }
        const safetyPassed = definition.expectedSafety === null ||
            actualSafety === definition.expectedSafety;
        const passed = toolSelectionPassed && groundingProxyPassed && safetyPassed;
        return {
            id: definition.id,
    ...
    const expectedBlocks = cases.filter((item) => item.expectedSafety === 'block');
    const correctlyBlocked = expectedBlocks.filter((item) => item.actualSafety === 'block').length;
    return {
        ...
        passCount,
        toolSelectionAccuracy: cases.filter((item) => item.toolSelectionPassed).length / cases.length,
        safetyBlockRecall: correctlyBlocked / expectedBlocks.length,
        groundingProxyRate: cases.filter((item) => item.groundingProxyPassed).length / cases.length,
